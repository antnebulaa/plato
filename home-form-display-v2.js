// SCRIPT DE BASE FONCTIONNEL (adapté pour utiliser votre XanoClient et avec des logs)

// Assurez-vous que la définition de votre classe XanoClient est présente AVANT ce script.
// Si elle est dans home-form-display-v2-2.txt, vous devrez la copier.

// Assurez-vous que getQueryParam et getCookie sont définies (elles le sont dans votre script fonctionnel)

document.addEventListener('DOMContentLoaded', function() {
    console.log("[SCRIPT_FONCTIONNEL] DOMContentLoaded");

    const xanoClient = new XanoClient({ // Utilisation de votre classe XanoClient
        apiGroupBaseUrl: 'https://xwxl-obyg-b3e3.p7.xano.io/api:qom0bt4V',
    });

    // Gérer les chargements de données API pour les listes d'annonces
    // On cible spécifiquement l'élément pour 'property/getall' pour cette page
    const listingElement = document.querySelector('[data-xano-endpoint="property/getall"]');
    
    if (listingElement) {
        console.log("[SCRIPT_FONCTIONNEL] Élément pour property/getall trouvé.");
        ...
        // Appel initial pour la liste (sans filtres)
        // Puisque property/getall est public, pas besoin de authToken pour cet appel spécifique.
        fetchXanoDataForList(xanoClient, endpoint, method, {}, listingElement); // {} = pas de filtres initiaux

        // --- PARTIE FILTRES (à ajouter/intégrer ici) ---
        // Voir Étape 3 ci-dessous

    } else {
        console.warn("[SCRIPT_FONCTIONNEL] Élément [data-xano-endpoint='property/getall'] non trouvé.");
    }

    // Gestion des liens génériques (hors liste) - repris de votre script fonctionnel
    document.addEventListener('xano:data-loaded', function(event) {
        // ... (copiez ici la section "Gérer les liens de navigation après le chargement des données"
        //      de votre script fonctionnel, lignes 30 à 61 de votre partage) ...
        // Attention: assurez-vous que les sélecteurs et la logique sont pertinents.
        // Pour faire simple pour l'instant, on peut la commenter si elle n'est pas cruciale pour la liste.
        console.log("[SCRIPT_FONCTIONNEL] xano:data-loaded event (pour liens hors liste).", event.detail.element);
    });

    document.addEventListener('click', function(event) {
        // ... (copiez ici la section "Gérer les clics sur les éléments non-liens"
        //      de votre script fonctionnel, lignes 64 à 73 de votre partage) ...
        console.log("[SCRIPT_FONCTIONNEL] Click event pour data-xano-link-target.");
    });
});

async function fetchXanoDataForList(client, endpoint, method, params, targetElement) {
    console.log(`[FETCH] Appel à ${endpoint} avec params:`, JSON.stringify(params));
    ...
    let response; // Réponse brute de Xano
    try {
        if (method === 'GET') {
            response = await client.get(endpoint, params); // Méthode de VOTRE XanoClient
        } else {
            throw new Error(`Méthode HTTP non supportée pour fetchXanoDataForList: ${method}`);
        }
        console.log(`[FETCH] Réponse reçue de Xano pour ${endpoint}:`, JSON.stringify(response));
        renderDataFromFunctionalScript(response, targetElement); // Appel à la fonction de rendu adaptée

    } catch (error) {
        console.error(`[FETCH] Erreur Xano pour ${endpoint}:`, error);
        if (targetElement.querySelector('[data-xano-list-container]')) {
             targetElement.querySelector('[data-xano-list-container]').innerHTML = `<p style="color:red;padding:20px;text-align:center;">Erreur lors du chargement: ${error.message}</p>`;
        } else {
            targetElement.innerHTML = `<p style="color:red;padding:20px;text-align:center;">Erreur lors du chargement: ${error.message}</p>`;
        }
        targetElement.dispatchEvent(new CustomEvent('xano:data-error', { detail:{error: error, element: targetElement}, bubbles: true }));
    } finally {
        if (loadingIndicator) loadingIndicator.style.display = 'none';
    }
}

function renderDataFromFunctionalScript(data, element) { // Renommée pour clarté
    console.log("[RENDER_DATA] Données brutes reçues pour rendu:", JSON.stringify(data));
    if (element.hasAttribute('data-xano-list')) {
        let listData = null;
        
        // --- Logique de détection de listData (de votre script fonctionnel) ---
        if (Array.isArray(data)) { listData = data; }
        else if (data && Array.isArray(data.items)) { listData = data.items; } // Chemin le plus courant pour les listes Xano
        else if (data && data.body && Array.isArray(data.body.items)) { listData = data.body.items; }
        else if (data && data.body && Array.isArray(data.body)) { listData = data.body; }
            ...
            if (!listData && data.body && typeof data.body === 'object') {
                for (const key in data.body) { if (Array.isArray(data.body[key])) { listData = data.body[key]; break; } }
            }
        }
        // --- Fin de la logique de détection ---

        if (listData && Array.isArray(listData)) {
            console.log(`[RENDER_DATA] ${listData.length} items trouvés pour la liste.`);
            renderListDataFromFunctionalScript(listData, element); // Appel à la fonction de rendu de liste adaptée
        } else {
            console.warn('[RENDER_DATA] Données non valides ou tableau non trouvé pour la liste.', data);
            const container = element.querySelector('[data-xano-list-container]') || element;
            container.innerHTML = `<p style="color:orange;padding:20px;text-align:center;">Données de liste non valides ou non trouvées.</p>`;
        }
    } else {
        // Gestion d'un élément unique (moins pertinent pour property/getall mais présent dans le script fonctionnel)
        console.log("[RENDER_DATA] Rendu d'un élément unique (non-liste).");
        const dataToBind = data.body || data; // Souvent, les données d'un item unique sont dans .body
        element.querySelectorAll('[data-xano-bind]').forEach(boundElement => {
            bindDataToElement(boundElement, dataToBind); // Utilise VOTRE bindDataToElement COMPLET
        });
    }
    element.dispatchEvent(new CustomEvent('xano:data-loaded', { detail: { data: data, element: element }, bubbles: true }));
}

function renderListDataFromFunctionalScript(dataArray, element) { // Renommée pour clarté
    console.log(`[RENDER_LIST] Début du rendu pour ${dataArray.length} items.`);
    ...
    if (!templateElement) {
        console.error(`[RENDER_LIST] ERREUR: Template "${templateSelector}" introuvable.`);
        container.innerHTML = `<p style="color:red;padding:20px;text-align:center;">Erreur : Élément modèle "${templateSelector}" introuvable.</p>`;
        return;
    }

    // Nettoyage du conteneur (adapté de votre script fonctionnel)
    if (container !== element || (templateElement.parentNode !== container) ) {
        // Si container est un enfant spécifique, OU si le template n'est PAS un enfant direct du container principal, on peut vider simplement.
        container.innerHTML = '';
    } else {
        // Si le container EST l'élément principal ET que le template est un enfant direct,
        // on fait attention à ne pas supprimer le template lui-même lors du nettoyage.
        // Cette situation est moins idéale. Mieux vaut que le template soit hors du container d'items.
        console.warn("[RENDER_LIST] Nettoyage prudent car le template pourrait être un enfant direct du conteneur principal.");
    ...
    // Cacher le template s'il n'est pas une balise <template> et s'il est toujours dans le DOM visible
    if (templateElement.tagName !== 'TEMPLATE' && templateElement.parentNode) {
         templateElement.style.display = 'none';
    }


    if (dataArray.length === 0) {
        let emptyMsg = element.getAttribute('data-xano-empty-message') || "Aucune donnée à afficher.";
        container.innerHTML = `<div class="xano-empty-message" style="padding:20px; text-align:center;">${emptyMsg}</div>`;
        console.log("[RENDER_LIST] Affichage du message 'liste vide'.");
        return;
    }
    ...
    dataArray.forEach((item, index) => {
        ...
        if (templateElement.tagName === 'TEMPLATE') {
            clone = templateElement.content.firstElementChild.cloneNode(true);
        } else {
        ...
        // Lier les données avec VOTRE fonction bindDataToElement complète
        // Elle gère les data-xano-bind, data-xano-bind-attr-*, _property_photos_slider
        bindDataToElement(clone, item); // Appel principal sur le clone
        clone.querySelectorAll('*').forEach(childEl => { // Et sur tous les descendants
             // bindDataToElement doit vérifier si l'élément a des attributs de binding
             bindDataToElement(childEl, item);
        });
        
        // Gérer les liens DANS cet item (adapté de votre script fonctionnel)
        clone.querySelectorAll('[data-xano-link-to]').forEach(linkElement => {
            ...
            const idValue = getNestedValue(item, idField); // Utilisation de getNestedValue
            
            if (idValue !== undefined && idValue !== null) {
                const linkTarget = `${targetPage}?${idParamName}=${encodeURIComponent(idValue)}`;
    ...
    console.log(`[RENDER_LIST] ${dataArray.length} items clonés et ajoutés au DOM.`);

    // --- PARTIE SWIPER (à ajouter/intégrer ici) ---
    // Voir Étape 2 ci-dessous
    initializePageSwipers(container); // Appel à la fonction d'initialisation Swiper
...
// --- ÉTAPE 2 : INTÉGRATION DE L'INITIALISATION SWIPER ---
function initializePageSwipers(parentElement) {
    // Cette fonction est basée sur la logique Swiper de votre home-form-display-v2-2.txt
    // Elle est appelée APRES que les items ont été ajoutés au DOM par renderListDataFromFunctionalScript
    const slidersToInitialize = parentElement.querySelectorAll('.swiper[data-slider-init="true"]');
    console.log(`[SWIPER_INIT] ${slidersToInitialize.length} sliders marqués pour initialisation dans`, parentElement);

    slidersToInitialize.forEach((swiperEl, sliderIndex) => {
        console.log(`[SWIPER] Préparation slider #${sliderIndex}:`, swiperEl);
        // Utiliser requestAnimationFrame ou un court délai pour s'assurer que le DOM est stable
        requestAnimationFrame(() => { // Ou setTimeout si requestAnimationFrame pose problème
            try {
                ...
                swiperEl.removeAttribute('data-slider-init'); // Important!
                console.log(`[SWIPER] Slider #${sliderIndex} initialisé.`);
            } catch (swiperError) {
                console.error(`[SWIPER] Erreur sur slider #${sliderIndex}:`, swiperError, swiperEl);
            }
